"""Progress callback system for scraping operations."""

from typing import Any, Optional
import json
from datetime import datetime


class ProgressCallback:
    """Base callback class for progress tracking."""
    
    async def on_start(self, scraper_type: str, url: str) -> None:
        """
        Called when scraping starts.
        
        Args:
            scraper_type: Type of scraper (person, company, job, etc.)
            url: URL being scraped
        """
        pass
    
    async def on_progress(self, message: str, percent: int) -> None:
        """
        Called during scraping to report progress.
        
        Args:
            message: Progress message
            percent: Progress percentage (0-100)
        """
        pass
    
    async def on_complete(self, scraper_type: str, result: Any) -> None:
        """
        Called when scraping completes successfully.
        
        Args:
            scraper_type: Type of scraper
            result: The scraped result object
        """
        pass
    
    async def on_error(self, error: Exception) -> None:
        """
        Called when an error occurs.
        
        Args:
            error: The exception that occurred
        """
        pass


class ConsoleCallback(ProgressCallback):
    """Print progress to console."""
    
    def __init__(self, verbose: bool = True):
        """
        Initialize console callback.
        
        Args:
            verbose: If True, print all messages. If False, only print major milestones.
        """
        self.verbose = verbose
    
    async def on_start(self, scraper_type: str, url: str) -> None:
        """Print start message."""
        print(f"🚀 Starting {scraper_type} scraping: {url}")
    
    async def on_progress(self, message: str, percent: int) -> None:
        """Print progress message."""
        if self.verbose or percent % 20 == 0:  # Print every 20% if not verbose
            bar_length = 30
            filled = int(bar_length * percent / 100)
            bar = '█' * filled + '░' * (bar_length - filled)
            print(f"[{bar}] {percent}% - {message}")
    
    async def on_complete(self, scraper_type: str, result: Any) -> None:
        """Print completion message."""
        print(f"✅ Completed {scraper_type} scraping successfully!")
    
    async def on_error(self, error: Exception) -> None:
        """Print error message."""
        print(f"❌ Error: {error}")


class SilentCallback(ProgressCallback):
    """Silent callback that does nothing (for when you don't want progress updates)."""
    pass


class JSONLogCallback(ProgressCallback):
    """Log progress as JSON to a file."""
    
    def __init__(self, log_file: str):
        """
        Initialize JSON log callback.
        
        Args:
            log_file: Path to log file
        """
        self.log_file = log_file
        self.logs = []
    
    def _log(self, event_type: str, data: dict) -> None:
        """Add a log entry."""
        entry = {
            "timestamp": datetime.utcnow().isoformat(),
            "event_type": event_type,
            **data
        }
        self.logs.append(entry)
        
        # Append to file
        with open(self.log_file, 'a') as f:
            f.write(json.dumps(entry) + '\n')
    
    async def on_start(self, scraper_type: str, url: str) -> None:
        """Log start event."""
        self._log("start", {"scraper_type": scraper_type, "url": url})
    
    async def on_progress(self, message: str, percent: int) -> None:
        """Log progress event."""
        self._log("progress", {"message": message, "percent": percent})
    
    async def on_complete(self, scraper_type: str, result: Any) -> None:
        """Log completion event."""
        self._log("complete", {"scraper_type": scraper_type})
    
    async def on_error(self, error: Exception) -> None:
        """Log error event."""
        self._log("error", {"error": str(error), "error_type": type(error).__name__})


class MultiCallback(ProgressCallback):
    """Combine multiple callbacks."""
    
    def __init__(self, *callbacks: ProgressCallback):
        """
        Initialize multi-callback.
        
        Args:
            *callbacks: Variable number of callback instances
        """
        self.callbacks = callbacks
    
    async def on_start(self, scraper_type: str, url: str) -> None:
        """Call all callbacks."""
        for callback in self.callbacks:
            await callback.on_start(scraper_type, url)
    
    async def on_progress(self, message: str, percent: int) -> None:
        """Call all callbacks."""
        for callback in self.callbacks:
            await callback.on_progress(message, percent)
    
    async def on_complete(self, scraper_type: str, result: Any) -> None:
        """Call all callbacks."""
        for callback in self.callbacks:
            await callback.on_complete(scraper_type, result)
    
    async def on_error(self, error: Exception) -> None:
        """Call all callbacks."""
        for callback in self.callbacks:
            await callback.on_error(error)


class InMemoryProgressCallback(ProgressCallback):
    """Store progress in memory for frontend access."""
    
    # Thread-safe storage for progress updates
    _progress_store: dict[str, dict] = {}
    _lock = None
    
    def __init__(self, job_id: str):
        """
        Initialize in-memory progress callback.
        
        Args:
            job_id: Unique identifier for this scraping job
        """
        self.job_id = job_id
        # Initialize progress for this job
        if self._lock is None:
            import threading
            InMemoryProgressCallback._lock = threading.Lock()
        
        with InMemoryProgressCallback._lock:
            InMemoryProgressCallback._progress_store[job_id] = {
                'percent': 0,
                'message': 'Initializing...',
                'status': 'in_progress',
                'error': None
            }
    
    async def on_start(self, scraper_type: str, url: str) -> None:
        """Update start status."""
        with InMemoryProgressCallback._lock:
            if self.job_id in InMemoryProgressCallback._progress_store:
                InMemoryProgressCallback._progress_store[self.job_id].update({
                    'percent': 0,
                    'message': f'Starting {scraper_type} scraping...',
                    'status': 'in_progress'
                })
    
    async def on_progress(self, message: str, percent: int) -> None:
        """Update progress - ensures monotonic increase (never goes backwards)."""
        with InMemoryProgressCallback._lock:
            if self.job_id in InMemoryProgressCallback._progress_store:
                current_percent = InMemoryProgressCallback._progress_store[self.job_id].get('percent', 0)
                # Only update if new percent is greater than current (monotonic increase)
                # This prevents progress bar from going backwards
                new_percent = min(100, max(0, percent))
                if new_percent > current_percent:
                    InMemoryProgressCallback._progress_store[self.job_id].update({
                        'percent': new_percent,
                        'message': message,
                        'status': 'in_progress'
                    })
                else:
                    # If percent is same or less, just update message but keep current percent
                    InMemoryProgressCallback._progress_store[self.job_id].update({
                        'message': message,
                        'status': 'in_progress'
                    })
    
    async def on_complete(self, scraper_type: str, result: Any) -> None:
        """Update completion status."""
        with InMemoryProgressCallback._lock:
            if self.job_id in InMemoryProgressCallback._progress_store:
                InMemoryProgressCallback._progress_store[self.job_id].update({
                    'percent': 100,
                    'message': 'Scraping completed successfully!',
                    'status': 'completed'
                    # Note: result is stored separately in the thread function, not here
                })
    
    async def on_error(self, error: Exception) -> None:
        """Update error status."""
        with InMemoryProgressCallback._lock:
            if self.job_id in InMemoryProgressCallback._progress_store:
                InMemoryProgressCallback._progress_store[self.job_id].update({
                    'percent': InMemoryProgressCallback._progress_store[self.job_id].get('percent', 0),
                    'message': f'Error: {str(error)}',
                    'status': 'error',
                    'error': str(error)
                })
    
    @classmethod
    def get_progress(cls, job_id: str) -> Optional[dict]:
        """Get current progress for a job."""
        if cls._lock is None:
            return None
        with cls._lock:
            return cls._progress_store.get(job_id)
    
    @classmethod
    def clear_progress(cls, job_id: str) -> None:
        """Clear progress for a job (cleanup)."""
        if cls._lock is None:
            return
        with cls._lock:
            cls._progress_store.pop(job_id, None)