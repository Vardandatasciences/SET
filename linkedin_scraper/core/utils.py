...
async def detect_rate_limit(page: Page) -> None:
    ...
    # Check for specific rate limit error pages/messages
    # Only check for very specific rate limit indicators, not generic phrases
    try:
        # Check for specific error page indicators
        error_indicators = [
            'h1:has-text("Too many requests")',
            'h1:has-text("Rate limit")',
            '[data-test-id="rate-limit-error"]',
            '.error-page',
        ...
        for indicator in error_indicators:
            ...
            if count > 0:
                raise RateLimitError(
                    "Rate limit detected on LinkedIn.",
                    suggested_wait_time=1800  # 30 minutes
                )
        
        # Check page title for rate limit
        title = await page.title()
        if title and ('rate limit' in title.lower() or 'too many requests' in title.lower()):
            raise RateLimitError(
                "Rate limit detected (page title indicates rate limiting).",
                suggested_wait_time=1800
            )
        
        # Only check body text if page is very short (likely an error page)
        # Normal LinkedIn pages are long, error pages are short
        body_text = await page.locator('body').text_content(timeout=2000)
        if body_text:
            body_lower = body_text.lower()
            body_length = len(body_text)
            
            # Only flag if it's a short page (likely error page) AND contains rate limit phrases
            if body_length < 500 and any(phrase in body_lower for phrase in [
                'too many requests',
                'rate limit exceeded',
                'slow down',
                'try again later',
        ...
        # Don't fail on other exceptions during rate limit check
        logger.debug(f"Error during rate limit check: {e}")
        pass
